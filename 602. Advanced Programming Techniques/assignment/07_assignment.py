...
plotly.tools.set_credentials_file(username='myvioletrose', api_key='JObGzxhZVhUA8tBNGF5c')
...
def exercise01():
    ...
    df_first_five_rows = pd.DataFrame(iris.data, columns = iris.feature_names)[:5]
...
def exercise02(new_observations):
    ...
    iris_predictions = classifier.predict(new_observations)
...
def exercise04():
    ...
    # Loop over different values of k (the code is similar to how we would do crossvalidation)
    ...
    # Create a trace using tutorial https://community.plot.ly/t/adding-traces-to-subplots/4424
    #https://plot.ly/python/line-charts/
    trace0 = plotly.graph_objs.Scatter(y = test_accuracy, x = neighbors, mode = 'lines', name = 'Testing Accuracy')
   
    trace1 = plotly.graph_objs.Scatter(y = train_accuracy, x = neighbors, mode = 'lines', name = 'Training Accuracy')
    
    data = [trace0, trace1]
        
    layout = dict(title = 'KNN: Number of Neighbors', xaxis = dict(title = 'Number of Neighbors'), yaxis = dict(title = 'Model Accuracy'),)

    plot = dict(data=data, layout=layout)
    
    plotly_overfit_underfit_curve_url = plotly.plotly.plot(plot)
...
def exercise05():
    ...
    feature_name = pd.DataFrame(boston.data, columns = boston.feature_names) 
    
    target_values = pd.DataFrame(boston.target) 
    
    average_age = feature_name['AGE'].mean() 
    
    medv_as_numpy_values = target_values.values 
...
def exercise06():
    ...
    PTRATIO = pd.DataFrame(boston.data, columns = boston.feature_names)['PTRATIO'] 
    
    X_ptratio = np.array(PTRATIO)
    
    plt.scatter(X_ptratio, boston.target)
...
def exercise07():
    ...
    PTRATIOb = pd.DataFrame(boston.data, columns = boston.feature_names)['PTRATIO']
    
    X_ptratio = np.array(PTRATIOb) 
    ...
    plt.plot(prediction_space, reg_model.predict(prediction_space), color='red', linewidth=3.5)
