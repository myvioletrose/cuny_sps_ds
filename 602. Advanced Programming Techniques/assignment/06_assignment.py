...
def exercise01():
    ...
    df = pd.DataFrame([[1,2,3,4], [4,5,6,7], [7,8,9,10]], columns = list('ABCD'))
...
def exercise05(n):
    ...
    zeros = np.array([[0] * n for i in range(n)])
...
def exercise06(n):
    ...
    ones = np.array([[1] * n for i in range(n)])
...
def exercise07(sd,m,s):
    ...
    random_numbers = np.array(np.random.normal(m, sd, s))
...
def exercise09():
    ...
    # adjust the url with suggested date range
    ...
    r2 = pd.DataFrame.from_dict(r1, orient = 'columns')
    
    df = r2.filter(items=['bpi'])
    
    df.bpi = df.bpi.astype(str)
    
    df = df[:-2]  # drop the last two rows
    
    df_test = df["bpi"].str.split('  ', n = 1, expand = True) 
...
def exercise12(n):
    ...
    for i in range(2*n): 
        ...
        for j in range(2*n): 
            ...
            print(checkerboard_matrix[i][j], end =" ")  
