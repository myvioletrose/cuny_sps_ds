...
class PandasChain:
    ...
    def __commit_block(self, block): 
        # Add code here
        block_hash = self.__chain.append(block)  # Create block hash
        # Add code here
        self.block_hash = self.block_hash.update(str(self.__id) + str(self.__seq_id) + str(self.__prev_hash) + str(self.__merkle_tx_hash) + str(self.__transactions))
...
class Block:
    ...
    def __init__(self,seq_id,prev_hash): 
        ...
        self.__status = 'UNCOMMITTED' or 'COMMITTED'  # Initial status. This will be a string.
    ...
    def add_transaction(self, s, r, v): 
        ...
        tx_hash = hashlib.sha256(str(str(uuid.uuid4()) + self.__name + str(dt.datetime.now())).encode('utf-8')).hexdigest()  # Hash of timestamp, sender, receiver, value
        new_transaction = pd.DataFrame(self.__transactions)  # Create DataFrame with transaction data (a DataFrame with only 1 row)        
    ...
    def get_simple_merkle_root(self): 
        pass
        merkle_root = str(self.tx_hash) + str(self.__block_hash) + str(self.__prev_hash)
        return merkle_root
