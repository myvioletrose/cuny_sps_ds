...
```{r area, eval=TRUE, echo = F}
summary(area)
hist(area)
```

1.  Describe this population distribution.
JN: The distribution appears to look symmetrical; however, it's slightly skewed to the right with mean larger than median. 
...
# the mean of samp1 (1593.7) is larger than mean of samp2 (1531.08); both are larger than the mean of population

set.seed(1234)
samp3 <- sample(area, 100); samp4 <- sample(area, 1000)
library(purrr)
map(1:5, function(x) mean(list(area, samp1, samp2, samp3, samp4)[[x]]))
# the largest sample (samp4 with 1000 random samples from the population) has the closest mean to the population
...
```
The shape of the sampling distribution resembles a normal distribution. The shape and spread look normal with center around 181000 with few extreme outliners on the right side. The mean home price (population) should be very close to the mean of sampling distribution. In fact, the mean of sampling distribution (with seed 1234) is 180955.9, whereas the population mean is 180796.1.
