...
```
# (1) we have 20000 cases and 9 variables. genhlth is categorical (ordinal); exerany, hlthplan, and smoke100 are discrete (binary); height, weight, wtdesire, and age are discrete; and gender is categorical (nominal).
...
```{r weight-mean-var-median, eval=F}
mean(cdc$weight) 
var(cdc$weight)
median(cdc$weight)
```
...
```{r table-smoke, eval = F}
table(cdc$smoke100)
```

or instead look at the relative frequency distribution by typing

```{r table-smoke-prop, eval = F}
...
```{r table-smoke-barplot-twosteps, eval = F}
...
```
# (2) please see summary, IQR and tables from above. There are 9569 males and 23% reported that they are in excellent health.
...
# (3) Males are more likely to smoke than females.
...
```{r names-again, eval = F}
names(cdc)
```

To see the weights for the first 10 respondents we can type

```{r first-10-rows-sixth-column, eval = F}
cdc[1:10,6]
```
...
```{r numbers-1to10, eval = F}
1:10
```

Finally, if we want all of the data for the first 10 respondents, type

```{r first-10-rows, eval = F}
cdc[1:10,]
```
...
# (4) under23_and_smoke <- with(cdc, cdc[age < 23 & smoke100 == 1, ])
...
```{r boxplot-height-gender, eval = F}
...
# (5) The "genhlth" boxplot displays a bmi for each genhlth value (from excellent to poor). Though they don't seem to display huge differences, we can see a trend that people reported having poorer health tended to have higher bmi values and wider the whiskers (meaning bigger data variation). On the other hand, I created above boxplot using gender. I speculated that gender can significantly impact bmi because of different diet and exercise patterns between men and women. The above boxplot does not seem to support the hypothesis. The boxplot seems to be highly similar between m and f, except f has wider whiskers (showing more variation within group - there's an extreme outliner with a bmi value above 70). Perhaps we can control age and other lifestyle patterns before running the comparison again for gaining better insight of bmi and gender.
...
```
# There's a linear relationship between the two variables, but people tend to desire about one-third less than their actual weight, e.g. lm(wtdesire ~ weight, data = cdc), coefficient for weight is about 0.639.
...
# "wdiff" is a discrete variable. If that is 0, that means there's no discrepancy between actual and desired weight. If the difference is positive, that means the person wants to be heavier; if negative, the person wants the opposite and he/she desires to lose more weight.
...
```
# The histogram (overlayed by a density curve) displays the distribution of wdiff, whereas the boxplot shows the same data with the extra display of outliners. The above summary shows basic statistics of this variable. The distribution slightly skewed to the right (e.g. median < mean, some extreme values on the far right). It has a center (using median) of -10, meaning people tend to desire less weight. The histogram and density curve display a leptokurtic distribution. It's best to focus on the IQR (so that we can ignore the extreme values on both ends) and see what the majority think about their weight in general. The IQR of 21 (from 75% quartile of 0 to 25% quartile of -21) indicates that people tend to desire less weight, i.e. 75% of samples in this study have wdiff 0 or less as a good indicator of majority desires to weight less, the IQR of 21 implies how much less they are desiring in general.
...
```
# there's no distinguishable difference between men and women on desired weight. Men have few more extreme values on the upper end, driving their mean higher than women (-10.7 vs -18.2). The median of men (-5) is slightly higher than women (-10) signals that although both men and women desire to weight less than actual, it's likely that women have stronger desire to have more weight loss than men.
...
cdc %>% 
        ...
        prop.table
```
# mean is equal to 169.683, whereas sd is equal to 40.08097. About 71% is within one standard deviation of the mean.
